...
@Suite struct SelectionPolicyTests {
    ...
    @Test func remembered_wins_when_it_exists() {
        ...
        #expect(SelectionPolicy.resolve(remembered: "a", groups: groups) == "a")
    }

    @Test func first_playing_when_remembered_is_gone() {
        let groups = [group("a", .idle), group("b", .playing), group("c", .playing)]
        #expect(SelectionPolicy.resolve(remembered: "zzz", groups: groups) == "b")
        #expect(SelectionPolicy.resolve(remembered: nil, groups: groups) == "b")
    }

    @Test func first_group_when_nothing_plays() {
        let groups = [group("a", .idle), group("b", .paused)]
        #expect(SelectionPolicy.resolve(remembered: nil, groups: groups) == "a")
    }

    @Test func nil_when_no_groups() {
        #expect(SelectionPolicy.resolve(remembered: "a", groups: []) == nil)
    }
}
