...
14. Updates: build with `MARKETING_VERSION` set below the latest release (`xcodebuild … MARKETING_VERSION=0.0.1`); within 30 s of launch the main screen shows "Update available — v<latest>". "Copy Homebrew command" swaps to "Copied" and pasting in Terminal gives `brew update && brew upgrade --cask remote-for-sonos`; "What's new" opens the release page; X hides the card and Settings → Version still says "Update available: <latest>" with the copy button. Switch "Check for updates" off → the card and the Settings line disappear; on again → they return. VoiceOver announces "Update available, version <latest>" once and reads the three buttons by name.
